...
setwd("~/AB109Cartograms/") # set the working directory
...
names(allCountiesData) <- str <- gsub("[^[:alnum:]]", "", names(allCountiesData))
...
ggplot(cartogramSplists, aes(long, lat, group=group)) +
  geom_polygon(aes(), fill="grey40", colour="grey90", alpha=.7, size = .05) +
  coord_map() +
  theme_minimal() # +
  # ggtitle("Cartogram of 1170h population by county")
...
ggplot(cartogramPSPs, aes(long, lat, group=group)) +
  geom_polygon(aes(), fill="grey40", colour="grey90", alpha=.7, size = .05) +
  coord_map() +
  theme_minimal() # +
  # ggtitle("Cartogram of PRCS population by county") 
```


```{r map of counties, fig.width= 7, fig.height = 9, fig.cap= "Map of California counties", message = FALSE, warning = FALSE, include=TRUE} 

ggplot(califCounties, aes(long, lat, group=group)) +
  geom_polygon(aes(), fill="grey40", colour="grey90", alpha=.7, size = .05) +
  coord_map() +
  theme_minimal() # +
...
```{r map of counties pop, fig.width= 7, fig.height = 9, fig.cap= "Cartogram of California counties population", message = FALSE, warning = FALSE, include=TRUE} 

ggplot(cartogramPopulation, aes(long, lat, group=group)) +
  geom_polygon(aes(), fill="grey40", colour="grey90", alpha=.7, size = .05) +
  coord_map() +
  theme_minimal() # +
  # ggtitle("California Counties") 
